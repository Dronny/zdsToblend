...
def Load_F(filename):
    tmp_data, object_list = [], []
    with open(filename) as f:
        tmp_data = f.readlines()
    for i in tmp_data:
        if "New object" in i: object_list.append([])
        if not len(object_list) == 0: object_list[-1].append(i)
    class dmd_model:
    ...
    for tmp_data in object_list:
        ...
        if texture_inited:
            ...
            for i in range(model.texture.vertnum):
                ...
                trimmed_uv = tuple((tmp_uv_tuple[0], 1 - tmp_uv_tuple[1])) # Cut_off 3-th coordinate
...
filename = r"D:\Program Files\ZDSimulator\routes\Москва-Калуга2_1_0\models\ForestL99.dmd"
Load_F(filename)
